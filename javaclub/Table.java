...
public class Table {
	...
	public int[] getRow(String field_name) throws NoSuchFieldException {
		...
		if (this.record.containsKey(field_name)) {
			return this.record.get(field_name);
		} else {
			throw new NoSuchFieldException(String.format("Key %s not found in record.", field_name));
		}
	...
	public void addRow(String field_name, int[] data) {
		...
		this.record.put(field_name, data);
	}
	...
	public void setVal(String field_name, String header, int val) {
		...
		try {
			int[] newRow = getRow(field_name);
			int headIndex = -1;

			for (int i = 0; i < this.n_cols; i++) {
				if (this.headers[i].equals(header)) headIndex = i;
			}
			if (headIndex != -1) {
				newRow[headIndex] = val;
				this.record.remove(field_name);
				addRow(field_name, newRow);
			}
